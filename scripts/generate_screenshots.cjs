...
async function create(spec) {
  ...
  try {
    if (typeof Jimp === 'function') {
      image = new Jimp(spec.w, spec.h, spec.bg);
    } else if (typeof Jimp.create === 'function') {
      image = await Jimp.create(spec.w, spec.h, spec.bg);
    } else if (typeof Jimp.read === 'function') {
      // fallback: create a blank image via read with a blank buffer option
      image = await Jimp.read({ create: { width: spec.w, height: spec.h, background: spec.bg } });
    } else {
      throw new Error('Unsupported Jimp API');
    }
  } catch (err) {
    throw err;
  }
